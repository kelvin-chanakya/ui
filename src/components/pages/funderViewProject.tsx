...
const ProjectList = () => {
  ...
  const cookies = new Cookies();
   useEffect(() => {
        async function fetchData() {
          let {data} = await axios.post(`http://ec2-18-224-65-21.us-east-2.compute.amazonaws.com:8080/ngo/getProjects`, {type: cookies.get('type')})
          setListData(data);
        }    
        fetchData()
      }, [])
  ...
  return (
      <Card title=" Projects" extra={<Button  onClick={logout} size = {'large'} >
      Logout
     </Button>}>
         {
              ...
              hoverable={true}
                    ...
                    cover={
                           ...
                           actions={[<FundViewOutlined key="view" onClick={()=>{
                            ...
                            message.loading('Adding to my Projects');    
                            let {data} = await axios.post(`http://ec2-18-224-65-21.us-east-2.compute.amazonaws.com:8080/project/updateProject`, {projectId: item._id, funderId: cookies.get('ngoId')})     
