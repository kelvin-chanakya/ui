...
const ProjectForm = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      form.setFieldsValue({targetBeneficiary: data.targetBeneficiary})
      form.setFieldsValue({location: data.location})
      form.setFieldsValue({objectives: data.objectives})
      form.setFieldsValue({activities: data.activities})
      form.setFieldsValue({duration: data.duration})
      form.setFieldsValue({durationUnit: data.durationUnit})
      form.setFieldsValue({totalBudget: data.totalBudget});

      form.setFieldsValue({status: data.status});

      form.setFieldsValue({problem: {
        description: _.get(data,'problem.description')
      }});

      form.setFieldsValue({strategy: {
        impactMonitoring: _.get(data,'strategy.impactMonitoring'),
        sustainability: _.get(data,'strategy.sustainability'),
        stakeHolder: _.get(data,'strategy.stakeHolder'),
        risks: _.get(data,'strategy.risks')
      }});

      form.setFieldsValue({solution: {
        description: _.get(data,'solution.description')
      }});

      form.setFieldsValue({benefits: {
        offerDescription: _.get(data,'benefits.offerDescription')
      }});
    
      setProjectLogo(data.logo);
      setBaselineStudyFileChange(_.get(data,'problem.baselineStudyFile'))
      setTocFileChange(_.get(data,'solution.tocFile'))

      setKpi(data.kpi);
      setBudget(data.budget);
      setImplementation(data.implementation);
      
      setKpiUrl(data.kpiUrl);
      setBudgeturl(data.budgetUrl);
      setImplementationUrl(data.implementationUrl);
  ...
  const onFinish = async (values: any) => {
    ...
    let { data } = await Axios.post(
      'http://ec2-18-224-65-21.us-east-2.compute.amazonaws.com:8080/project/updateProject',
      request
    );
